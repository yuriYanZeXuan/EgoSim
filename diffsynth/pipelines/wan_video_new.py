...
class WanVideoPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        # Redirect model path (only for relative paths)
        if redirect_common_files:
            ...
            for model_config in model_configs:
                if model_config.origin_file_pattern is None or model_config.model_id is None:
                    continue
                # Skip redirect if using absolute path
                if isinstance(model_config.origin_file_pattern, str) and os.path.isabs(model_config.origin_file_pattern):
                    continue
                if model_config.origin_file_pattern in redirect_dict and model_config.model_id != redirect_dict[model_config.origin_file_pattern]:
                    print(f"To avoid repeatedly downloading model files, ({model_config.model_id}, {model_config.origin_file_pattern}) is redirected to ({redirect_dict[model_config.origin_file_pattern]}, {model_config.origin_file_pattern}). You can use `redirect_common_files=False` to disable file redirection.")
                    model_config.model_id = redirect_dict[model_config.origin_file_pattern]
        
        # Extract tokenizer config from model_configs if provided
        # Look for any config with model_id containing "Wan2.1-T2V-1.3B" and origin_file_pattern ending with "google"
        extracted_tokenizer_config = None
        for model_config in model_configs:
            # Strip whitespace from model_id and origin_pattern
            model_id = model_config.model_id.strip() if model_config.model_id else ""
            origin_pattern = str(model_config.origin_file_pattern).strip() if model_config.origin_file_pattern else ""
            
            # Check if this is a tokenizer config
            # Match: model_id is "Wan-AI/Wan2.1-T2V-1.3B" AND path ends with "/google" or is "google"
            is_tokenizer = (
                model_id == "Wan-AI/Wan2.1-T2V-1.3B" and 
                origin_pattern and
                (origin_pattern.endswith("/google") or 
                 origin_pattern.endswith("google") or
                 origin_pattern == "google" or
                 origin_pattern.endswith("/google/") or
                 "google/" in origin_pattern or
                 "/google/" in origin_pattern)
            )
            if is_tokenizer:
                extracted_tokenizer_config = model_config
                break
        
        if extracted_tokenizer_config is not None:
            tokenizer_config = extracted_tokenizer_config
        ...
        for model_config in model_configs:
            # Skip tokenizer config as it will be handled separately
            if model_config is tokenizer_config:
                continue
            
            # If using absolute path, directly set path without downloading
            if model_config.path is None and isinstance(model_config.origin_file_pattern, str) and os.path.isabs(model_config.origin_file_pattern):
                if "*" in model_config.origin_file_pattern:
                    # Handle glob pattern
                    model_config.path = glob.glob(model_config.origin_file_pattern)
                    if isinstance(model_config.path, list) and len(model_config.path) == 1:
                        model_config.path = model_config.path[0]
                else:
                    model_config.path = model_config.origin_file_pattern
            else:
                # Only call download_if_necessary for relative paths
        ...
        # If using absolute path, directly set path without downloading
        if tokenizer_config.path is None and isinstance(tokenizer_config.origin_file_pattern, str) and os.path.isabs(tokenizer_config.origin_file_pattern):
            if "*" in tokenizer_config.origin_file_pattern:
                tokenizer_config.path = glob.glob(tokenizer_config.origin_file_pattern)
                if isinstance(tokenizer_config.path, list) and len(tokenizer_config.path) == 1:
                    tokenizer_config.path = tokenizer_config.path[0]
            else:
                tokenizer_config.path = tokenizer_config.origin_file_pattern
        else:
        ...
        if audio_processor_config is not None:
            # If using absolute path, directly set path without downloading
            if audio_processor_config.path is None and isinstance(audio_processor_config.origin_file_pattern, str) and os.path.isabs(audio_processor_config.origin_file_pattern):
                if "*" in audio_processor_config.origin_file_pattern:
                    audio_processor_config.path = glob.glob(audio_processor_config.origin_file_pattern)
                    if isinstance(audio_processor_config.path, list) and len(audio_processor_config.path) == 1:
                        audio_processor_config.path = audio_processor_config.path[0]
                else:
                    audio_processor_config.path = audio_processor_config.origin_file_pattern
            else:
