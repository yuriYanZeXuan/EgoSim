...
class WanImageEncoder(torch.nn.Module):
    ...
    def encode_image(self, videos):
        # preprocess — videos: [B, C, H, W] or list of [C, H, W] / [1, C, H, W]
        size = (self.model.image_size,) * 2
        if isinstance(videos, (list, tuple)):
            videos = torch.cat([
                F.interpolate(
                    u if u.dim() == 4 else u.unsqueeze(0),
                    size=size,
                    mode='bicubic',
        ...
        # [-1,1] -> [0,1] then apply CLIP normalization
        videos = self.transforms.transforms[-1](videos.mul(0.5).add(0.5))
