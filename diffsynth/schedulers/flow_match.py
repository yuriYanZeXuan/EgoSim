...
class FlowMatchScheduler():
    ...
    def add_noise(self, original_samples, noise, timestep):
        ...
        if isinstance(timestep_cpu, torch.Tensor) and timestep_cpu.dim() > 0:
            distances = (self.timesteps.unsqueeze(0) - timestep_cpu.unsqueeze(1)).abs()
            timestep_id = torch.argmin(distances, dim=1)
            sigma = self.sigmas[timestep_id]
    ...
    def training_weight(self, timestep):
        ...
        if timestep.dim() > 0:
            distances = (self.timesteps.unsqueeze(0) - timestep.unsqueeze(1)).abs()
            timestep_id = torch.argmin(distances, dim=1)
