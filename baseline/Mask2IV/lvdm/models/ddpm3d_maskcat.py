...
from utils.metrics.calculate_viclip import calculate_viclip
from utils.metrics.calculate_metric import calculate_metric
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.mask_attn_key = mask_attn_key
    ...
    def get_loss(self, pred, target, mean=True, mask=None, fg_weight=5):
        if self.loss_type == 'l1':
            ...
            if mean:
                ...
                if mask is not None:
                    print('contact weighted mse loss')
                    loss = (target - pred) * ((mask * (fg_weight - 1)) + 1)
                    loss = loss ** 2
                else:
...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        # mask_sum = mask.sum(1)
        mask = kwargs['target_mask']
        mask_down = F.interpolate(mask, scale_factor=1/8, mode='nearest').unsqueeze(1)
        ...
        loss_simple = self.get_loss(model_output, target, mean=False, mask=None, fg_weight=5).mean([1, 2, 3, 4])
        ...
        if self.original_elbo_weight != 0.0:
            loss_vlb = self.get_loss(model_output, target, mean=False, mask=None, fg_weight=5).mean(dim=(1, 2, 3, 4))
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def shared_step(self, batch, random_uncond, **kwargs):
        x, c, fs = self.get_batch_input(batch, random_uncond=random_uncond, return_fs=True, return_mask_attn=False)
        kwargs.update({"fs": fs.long()})
        kwargs.update({"target_mask": batch['contact_maps']})
        loss, loss_dict = self(x, c, mask=False, **kwargs)
        return loss, loss_dict
    
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_cond_frame=False, return_original_input=False, return_mask_attn=False, return_obj_info=False, **kwargs):
        ## x: b c t h w
        x = super().get_input(batch, self.first_stage_key)
        
        if self.first_stage_extra_key is not None:
            x_extra = super().get_input(batch, self.first_stage_extra_key)
            z_extra = self.encode_first_stage(x_extra)
        ...
        if return_mask_attn:
            mask_attn = super().get_input(batch, self.mask_attn_key)
            out.append(mask_attn)
    ...
    def validation_epoch_end(self, outputs):
        ...
        captions = [item for x in outputs for item in x['caption']]  # list of lists
        
        # compute metrics
        x_samples, x_recs = self.val_preprocess(x_samples), self.val_preprocess(x_recs)
        results = calculate_metric(x_samples, x_recs, captions)
        self.log_dict(results, prog_bar=True, logger=True, sync_dist=True)
        
        log_info = ""
        for key, value in results.items():
            log_info += f"{key}: {value:.4f} | "
        mainlogger.info(f"Validation results for {self.current_epoch}_{self.global_step}: {log_info[:-3]}")
