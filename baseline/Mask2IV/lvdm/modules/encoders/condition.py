...
from transformers import T5Tokenizer, T5EncoderModel, CLIPTokenizer, CLIPTextModel, AutoImageProcessor, Dinov2Model
...
class FrozenDINOv2Embedder(AbstractEncoder):
    """
    Uses the OpenCLIP vision transformer encoder for images
    """

    def __init__(self, model_name="facebook/dinov2-large", device="cuda", freeze=True, antialias=True):
        super().__init__()
    ...
    def forward(self, x):
        ## image: b c h w
        x = self.preprocess(x)
        x = self.model(x)["last_hidden_state"]    # bs x 257 x 768 / 1024 / 1536 - B/L/G
        return x