    ...
    def forward(self, x, masking_prob=0.0):
        ...
        if hasattr(self, 'temporal_positional_embedding'):
            if x.size(1) == 1:
                ...
                # Add interpolation if the number of frames is different
                if x.size(1) != self.temporal_positional_embedding.size(1):
                    tp_embed = self.temporal_positional_embedding
                    tp_embed = tp_embed.permute(0, 2, 1)  # [1, D, 8]
                    tp_embed = F.interpolate(tp_embed, size=16, mode='linear', align_corners=False)
                    tp_embed = tp_embed.permute(0, 2, 1)  # [1, 16, D]
                    x = x + tp_embed
                else:
