...
def log_local(batch_logs, save_dir, filename, save_fps=10, rescale=True, to_img=False):
    ...
    for key in batch_logs:
        if to_img and key not in ['samples', 'reconst', 'caption', 'image_condition', 'video']:
            continue
        if key not in ['samples', 'reconst', 'caption', 'image_condition', 'video']:
            continue

        value = batch_logs[key]
        if isinstance(value, list) and isinstance(value[0], str):
            ## a batch of captions
            path = os.path.join(save_dir, "%s/%s.txt"%(filename, key))
            ...
            if to_img:
                for i in range(grid.shape[0]):
                    path = os.path.join(save_dir, "%s/%d.png"%(filename, i))
                    os.makedirs(os.path.split(path)[0], exist_ok=True)
                    Image.fromarray(grid[i].cpu().numpy().astype(np.uint8)).save(path)
            
            if key == 'samples':
                path = os.path.join(save_dir, "%s/%s.mp4"%(filename, key))
            elif key == 'reconst':
                path = os.path.join(save_dir, "%s/%s-gt.mp4"%(filename, key))
            elif key == 'video':
                path = os.path.join(save_dir, "%s/%s-mask.mp4"%(filename, key))
            ...
            path = os.path.join(save_dir, "%s/%s.jpg"%(filename, key))
            save_img_grid(grid, path, rescale)
        else:
            pass
