...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--seed", "-s", type=int, default=42, help="seed for seed_everything")
    ...
    parser.add_argument("--tb_log", type=str, default="tb_logs", help="for tensorboard logs")
    ...
    parser.add_argument("--first_mask", "-f", type=str, default="")
    parser.add_argument("--to_local", action='store_true', default=False, help='save to local')
    parser.add_argument("--to_img", action='store_true', default=False, help='save as frames')
    parser.add_argument("--val_save_skip", action='store_true', default=False, help='do not save results during validation')
...
if __name__ == "__main__":
    ...
    if args.checkpoint and args.val:
        args.logdir = os.path.dirname(args.checkpoint).rsplit('/', 2)[0]
    ...
    ## load checkpoints
    ignore_keys = getattr(config.model.params, 'ignore_keys', None)
    if args.checkpoint:
        ...
        pl_sd = torch.load(config.model.pretrained_checkpoint, map_location="cpu")
        model.load_state_dict(pl_sd["state_dict"], strict=True)
        print(">>> Loaded weights from pretrained checkpoint: %s"%args.checkpoint)
    else:
        model = load_checkpoints(model, config.model, ignore_keys=ignore_keys)
    ...
    if args.first_mask:
        config.data.params.validation.params.first_mask_path = args.first_mask
    ...
    if args.tb_log:
        # TODO: hard code here, to make a new folder for tensorboard log
        workdir = workdir.replace('exp_outputs', 'exp_outputs/tb_logs')    
    logger_cfg = get_trainer_logger(lightning_config, workdir, args.val)
    trainer_kwargs["logger"] = instantiate_from_config(logger_cfg)
    
    ## setup callbacks
    callbacks_cfg = get_trainer_callbacks(lightning_config, config, workdir, ckptdir, logger)
    if args.to_local:
        callbacks_cfg.batch_logger.params['to_local'] = True
    if args.to_img:
        callbacks_cfg.batch_logger.params['to_img'] = True
    if args.val_save_skip:
        callbacks_cfg.batch_logger.params['val_batch_frequency'] = -1
